...
public:
                               ...
                               int seed = 42)
    {
        ...
        class0_idx.reserve(X.rows());
        class1_idx.reserve(X.rows());

        for (int i = 0; i < y.size(); i++) {
            if (y(i) == 0) class0_idx.push_back(i);
            else class1_idx.push_back(i);
        }

        mt19937 rng(seed);
        shuffle(class0_idx.begin(), class0_idx.end(), rng);
        shuffle(class1_idx.begin(), class1_idx.end(), rng);

        int test0 = (int)(class0_idx.size() * test_ratio);
        int test1 = (int)(class1_idx.size() * test_ratio);

        vector<int> test_idx;
        vector<int> train_idx;
        test_idx.reserve(test0 + test1);
        train_idx.reserve(X.rows() - test0 - test1);

        test_idx.insert(test_idx.end(), class0_idx.begin(), class0_idx.begin() + test0);
        test_idx.insert(test_idx.end(), class1_idx.begin(), class1_idx.begin() + test1);
        train_idx.insert(train_idx.end(), class0_idx.begin() + test0, class0_idx.end());
        train_idx.insert(train_idx.end(), class1_idx.begin() + test1, class1_idx.end());

        shuffle(test_idx.begin(), test_idx.end(), rng);
        shuffle(train_idx.begin(), train_idx.end(), rng);

        int test_size  = (int)test_idx.size();
        int train_size = (int)train_idx.size();
...
int main(int argc, char* argv[]) {
    ...
    string csv_path = (argc > 1) ? argv[1] : "data/mnist_binary_3_8.csv";
    ...
    ofstream results("results/resultados_cpp.txt");
    ...
    results.close();
    cout << "\n[OK] Resultados exportados a: results/resultados_cpp.txt\n";

    // ---- Exportar pesos (para visualización en Python) ----
    ofstream weights_file("results/pesos_cpp.csv");
    ...
    weights_file << "\n" << model.getBias() << "\n";
    weights_file.close();
    cout << "[OK] Pesos exportados a: results/pesos_cpp.csv\n";
